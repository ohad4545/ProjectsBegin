...
public static void option3(String userName)
{
	...
	else
	{
		...
		{
			/*Because ourLastDeliveries is sorted by lastName(compareTo in Members class),it means that a delivery can be added to that treeMap to the wrong member with the same lastName*/
			/*so,if a delivery is added to a member that has the same lastName with one of the members in the whole manager member's HashMaps,this specific member will be sorted by his id(unique property), what promises that the delivery will be added with the right member key*/
			...
			public int compareTo(Members m) {
				...
				DataBase.ourLastDeliveries.put(tmpMember2, tmpDelivery);
		}
...
public static void option5()
{
	...
	do
	{
		...
		{
			if(entry.getKey().getMemberId().equals(tmpMember.getMemberId()))
			{
