	...
	public static void main(String[] args) {
        ...
        while(user_choice != '8')
        {
    		...
    		while(user_choice != '4' && user_choice != '8')
    		{
    			...
    			System.out.println("5 - Watch member's last delivery and whole deliveries");
...
public static void option1(String userName)
{
	...
	do
	{
		do
		{
			...
			if(!(isValidLivingArea) || !(tmpMember.setMemberId(memberId, DataBase.getManagers(), DataBase.ManagerByUserName(userName))))
			{
...
public static void option3(String userName)
{
	...
	else
	{
		...
		do
		{
			...
			if(tmpDelivery.setDeliveryCode(deliveryCode, DataBase.getManagers()))
			{
		...
		DataBase.getOurLastDeliveries().put(tmpMember, tmpDelivery);
		DataBase.getOurMembersAndDeliveries1().put(DataBase.getCountTree(), tmpMember);
		DataBase.getOurMembersAndDeliveries2().put(DataBase.getCountTree(), tmpDelivery);
	...
	do
	{
		...
		for(Entry<Members,Delivery> entry: DataBase.ourLastDeliveries.entrySet())
		{
			if(entry.getKey().getMemberId().equals(memberId))
			{
				...
				/*this iterator represents the member's whole deliveries ArrayList*/
				Iterator<Delivery> itr = entry.getKey().whichManagerStrExists(memberId, DataBase.managers).getManagerMembers().get(memberId).getDeliveries().iterator();
				System.out.println("The whole member's deliveries are: ");
				while(itr.hasNext())
				{
