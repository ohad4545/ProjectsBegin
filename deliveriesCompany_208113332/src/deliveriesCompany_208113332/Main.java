...
public static void option3(String userName)
{
	...
	else
	{
		...
		/*I made this while loop because ourLastDeliveries is sorted by last name, if a delivery is added to one of the member's HashMap, it may be added to other member in ourLastDeliveries with the same last name */
		/*In that way I promise that the delivery will be added to the currect member by its id(unique property) and outLastDeliveries TreeMap will still be sorted by last name*/
		while(DataBase.sameLastName(tmpMember)) {
			tmpMember.setLastName(tmpMember.getLastName() + " ");
		}
		...
		System.out.println(DataBase.ourLastDeliveries);
